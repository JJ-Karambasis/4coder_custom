...
isearch(Application_Links *app, Scan_Direction start_scan, i64 first_pos,
    ...
    for (;;){
        ...
        if (!string_change){
            if ((match_key_code(&in, KeyCode_M) && (has_modifier(&in.event.key.modifiers, KeyCode_Control))) ||
                match_key_code(&in, KeyCode_PageDown) ||
                match_key_code(&in, KeyCode_Down)){
                change_scan = Scan_Forward;
                do_scan_action = true;
            }
            if ((match_key_code(&in, KeyCode_N) && (has_modifier(&in.event.key.modifiers, KeyCode_Control))) || 
