...
function b32
jj_handle_backspace(Application_Links* app, Lister* lister, Lister_Activation_Code* Code)
{
...
function b32
jj_handle_navigation(Application_Links* app, Lister* lister, View_ID view, Lister_Activation_Code* Code, i32 Offset)
{
...
jj_run_lister(Application_Links* app, Lister* lister)
{
    lister->filter_restore_point = begin_temp(lister->arena);
    lister_update_filtered_list(app, lister);
        ...
        switch (in.event.kind){
            ...
            case InputEventKind_KeyStroke:
            {
                switch (in.event.key.code){
                    ...
                    {                                                
                        handled = jj_handle_backspace(app, lister, &result);                                                                                                                                                
                    } break;
                    
                    case KeyCode_Up:                    
                    {                        
                        handled = jj_handle_navigation(app, lister, view, &result, -1);                                                
                    } break;
                    ...
                    case KeyCode_PageUp:
                    {
                        handled = jj_handle_navigation(app, lister, view, &result, -lister->visible_count);                        
                    } break;
                    ...
                    default:
                    {
                        if(has_code_and_modifier(&in.event, KeyCode_S, KeyCode_Control))                        
                            handled = jj_handle_backspace(app, lister, &result);                        
                        else if(has_code_and_modifier(&in.event, KeyCode_I, KeyCode_Control))                        
                            handled = jj_handle_navigation(app, lister, view, &result, -1);                        
                        else if(has_code_and_modifier(&in.event, KeyCode_K, KeyCode_Control))                        
                            handled = jj_handle_navigation(app, lister, view, &result, 1);                        
                        else
                        {                                                        
...
function Lister_Result
_run_lister(Application_Links *app, Lister *lister){
    lister->filter_restore_point = begin_temp(lister->arena);
    lister_update_filtered_list(app, lister);
...
run_lister_with_refresh_handler(Application_Links *app, Arena *arena, String_Const_u8 query, Lister_Handlers handlers){
    ...
    if (handlers.refresh != 0){
        ...
        result = jj_run_lister(app, lister);
    }
    else{
...
get_choice_from_user(Application_Links *app, String_Const_u8 query,
    ...
    Lister_Result l_result = jj_run_lister(app, lister);
