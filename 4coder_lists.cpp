...
function void
jj_lister_backspace_text_field_file_path(Application_Links *app){
    View_ID view = get_this_ctx_view(app, Access_Always);
    Lister *lister = view_get_lister(view);
    if (lister != 0){
        if (lister->text_field.size > 0){
            ...
            if (character_is_slash(last_char)){
                ...
                b32 is_modified = has_modifier(&input, KeyCode_Shift);                
                if (!is_modified){
...
function void
_lister__backspace_text_field__file_path(Application_Links *app){
    View_ID view = get_this_ctx_view(app, Access_Always);
    Lister *lister = view_get_lister(view);
    if (lister != 0){
        if (lister->text_field.size > 0){
...
get_file_name_from_user(Application_Links *app, Arena *arena, String_Const_u8 query,
    ...
    handlers.backspace = jj_lister_backspace_text_field_file_path;
